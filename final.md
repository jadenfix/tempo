...
- `tempo-engine-host` — out-of-proc engine host: driver wire protocol over UDS, crash isolation, N webviews/process. Engine lanes host the observation-compiler core before the daemon boundary, so only compiled diffs cross the process boundary. The compile locus and evidence bar are pinned in [`docs/OBSERVATION_COMPILE_LOCUS_ADR.md`](docs/OBSERVATION_COMPILE_LOCUS_ADR.md): the CDP preview lane is wired today; the native Servo/out-of-process lane is deferred until `tempo-engined` owns AccessKit-to-compiled-observation conversion engine-side. Verso's process split is the reference. The runnable host **shipped today** is the CDP-backed `tempo-engined-cdp` binary (in `tempo-engine-cdp`), which serves the CDP driver over the daemon's engine UDS (#397); the servo-hosted native `tempo-engined` is the WS2 target (see §3.3, below), not yet built.
...
| 5 | **Observation-pipeline latency / IPC overhead vs CDP** | CDP preview compiles before daemon IPC; native Servo keeps engine-side compilation as a gated lane requirement; diff-only transfer; perf budgets enforced in CI; speculative pre-observation during the quiescence wait |
