...
- [ ] Security/privacy docs do not rely on one top-level disclaimer to narrow later absolute claims. Wording such as "all", "every", "owns", "guarantees", or "resistance" carries local shipped-vs-roadmap scope where it appears.
- [ ] "Zero history", "zero trace", and "stealth" claims name the durable sinks covered by tests: journals, cassettes, telemetry, logs, engine profiles, browser cache/storage, crash reports, and OS temp files. OS, browser, network, or remote-service traces that remain outside Tempo's control are stated locally.
- [ ] Encryption claims name the protected data, key source, integrity mode, rotation/recovery behavior, and fail-closed handling for missing keys, unauthenticated data, and corrupt records. "Encrypted" alone is not enough to establish the security property.
- [ ] Browser stealth means local privacy, profile isolation, retention suppression, and policy enforcement. CAPTCHA solving, anti-bot bypass, and fingerprint-spoofing escalation require explicit safety/legal review and a separate product decision.
...
- [ ] Cassette and replay-fork import defaults fail closed to authenticated durable retention. Plaintext cassette helpers are compatibility/test fixtures only and must be explicitly named unsafe, never silent defaults for untrusted imports.
- [ ] A security bridge with live unit/canary tests is not automatically shipped composition. If runtime wiring is absent, docs and CI keep the capability deferred; if runtime wiring is added, a production-path sentinel proves the actual agent/headless/MCP/BiDi path.
...
- [ ] Cache and fast-path hooks that return snapshots, diffs, or observations document the authoritative identity they bind to, such as sequence, base sequence, URL, frame, policy epoch, or capability token. Callers reject cache misses or identity mismatches and re-derive state, with tests for stale and wrong-identity implementations.
...
- [ ] Crawl/network execution paths prefer connection-pinned capabilities with socket-safe request parts and already-open streams; any checked-dispatch fallback connects to the policy-checked socket, and raw scheduler dispatch is not handed to a client that can re-resolve the URL.
- [ ] Engine/control IPC fails closed on platforms without an implemented peer-auth transport; no permissive "local-only" fallback without same-user credentials, named-pipe ACLs, app-private sockets, or an explicit unsupported gate.
...
- [ ] Direct OpenAPI execution treats discovery as untrusted data unless a provenance gate says otherwise: safe-looking operations still require confirmation by default, preserve stronger local inference, and cannot copy model-supplied Authorization, cookie, API-key, token, secret, credential, or password-like parameters into requests without explicit secret bindings.
...
- [ ] Taint-to-beatbox claims are backed by an agent-facing runtime dispatch path, not only `tempo-toolexec` helper tests: page-tainted non-browser compute reaches live beatbox with `net:Deny`, `secrets:[]`, and no canary egress.
- [ ] Observation boundary claims are backed by engine-side compilation: raw AccessKit/AX trees and `tempo-observe` raw compiler inputs stay in engine adapters or fixtures, while `tempod`-side runtime wire surfaces carry only `CompiledObservation`/`ObservationDiff`.
